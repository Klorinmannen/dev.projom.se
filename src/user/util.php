...
class util {
    ...
    public static function add_new_user(string $username, string $password) {

        // At this point username and password should be ok.
    ...
    public static function create(string $username,
                                  string $password,
                                  bool $api_user = false,
                                  bool $auto_activate = false): array {

        $user['Name'] = $username;
        $user['Password'] = \user\password::hash($password);
        $user['Active'] = $auto_activate ? -1 : 0;
    ...
    public static function set_new_password(int $user_id, string $new_password = ''): array {
        if (!\util\validate::id($user_id))
            throw new \Exception('Missing/malformed user id', 400);
